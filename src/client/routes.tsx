...
import { REGIONAL_CODE } from '../types'
...
const Regionals = (region?: REGIONAL_CODE) =>
  React.lazy(() =>
    import('./views/regional/Regionals').then(({ default: EL }) => {
      return { default: () => <EL region={region} /> }
    }),
  )
...
const routes: Route[] = [
  {
    ...
    element: React.lazy(() => Promise.resolve({ default: () => <a>{Math.random()}</a> })),
  },
  {
    path: '/dashboard',
  ...
  ...Object.keys(REGIONAL_CODE).filter(name => Number.isNaN(parseInt(name))).map((name) => {
    return {
      path: `/regional/${name}`,
      name,
      element: Regionals(name as unknown as REGIONAL_CODE),
    }
  }),
]
